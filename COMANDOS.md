...
Comando	Função
git stash	Guarda mudanças não commitadas temporariamente sem commitar, para trocar de branch em segurança
git stash pop	Recupera o que foi guardado no stash
git clone <url>	Copia um repositório remoto para a máquina local
git fetch	Baixa referências do remoto sem aplicar no seu código (diferente de pull, que baixa + já mescla)
